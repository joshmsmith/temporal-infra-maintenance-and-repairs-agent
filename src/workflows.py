...
with workflow.unsafe.imports_passed_through():
    from src.activities import analyze, detect, plan_repair, notify, execute_repairs, report

ITERATIONS_BEFORE_CONTINUE_AS_NEW = 10  # Number of iterations before exiting the workflow

'''InfraMonAgentWorkflow: 
This is an agent implemented as a Temporal Workflow that orchestrates monitoring of infrastructure and scheduling repairs.
...
class InfraMonAgentWorkflow:
    ...
    async def run(self, inputs: dict) -> str:
        ...
        workflow.logger.debug(f"Starting infra monitoring workflow with inputs: {inputs}")
        ...
        if self.rejected:
            workflow.logger.info(f"Planned repair REJECTED by user {self.context.get('rejected_by', 'unknown')}")
            self.set_workflow_status("REJECTED")
            return f"Planned repair REJECTED by user {self.context.get('rejected_by', 'unknown')}"
        
        self.set_workflow_status("APPROVED")
        workflow.logger.info(f"Planned repair approved by user {self.context.get('approved_by', 'unknown')}")
...
'''InfraMonAgentWorkflowProactive: 
This is an agent implemented as a Temporal Workflow that orchestrates monitoring of infrastructure and scheduling repairs.
It's much like InfraMonAgentWorkflow, but it is designed to run proactively.
...
Tools are static Activities that implement automated helper agents.'''
@workflow.defn
class InfraMonAgentWorkflowProactive(InfraMonAgentWorkflow):
    def __init__(self) -> None:
        InfraMonAgentWorkflow.__init__(self)
    ...
    async def run(self, inputs: dict) -> str:
        ...
        while not self.exit_requested and not self.continue_as_new_requested and self.iteration_count < ITERATIONS_BEFORE_CONTINUE_AS_NEW:
            ...
            if self.continue_as_new_requested or self.exit_requested:
                ...
                # for anything but very high planning confidence scores, we will notify the user and wait for approval
